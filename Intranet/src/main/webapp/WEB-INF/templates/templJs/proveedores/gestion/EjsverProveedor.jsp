...
<script src="<c:url value="/resources/assets/js/bootstrap.js" />"></script>
    ...
    $(function(){
        $("#btn_desaprobar").click(function(){ 
            $.ajax({
                   type: "POST",
                   url: "/Intranet/emp/EajaxProvDesaprobado.htm",
         ...
         $("#btn_aceptarD").click(function(){ 
            window.location.href = "/Intranet/emp/EPverProveedor.htm?id="+${proveedor.id};
            // Evitar ejecutar el submit del formulario.
        });
        $("#btn_aprobar").click(function(){ 
            $.ajax({
                   type: "POST",
                   url: "/Intranet/emp/EajaxProvAprobado.htm",
         ...
         $("#btn_logeoProveedor").click(function(){ 
            $.ajax({
                   type: "POST",
                   url: "/Intranet/emp/EajaxLogeoProveedor.htm",
                   data: $("#formLP").serialize(), // Adjuntar los campos del formulario enviado.
                   success: function(data)
                   {
                        if(data!="0"){
        ...
        $("#btn_aceptarASinUsu").click(function(){ 
            $.ajax({
                   type: "POST",
                   url: "/Intranet/emp/EajaxProvAprobadoMUsu.htm",
                   data: $("#formASinUsu").serialize(), // Adjuntar los campos del formulario enviado.
                   success: function(data)
                   {
                        if(data!="0"){
                            window.location.href = data;
        ...
        $("#btn_aceptarACUsu").click(function(){ 
            $("#myModalUpdUsu").modal('show');
        });
        $("#btn_logeoProveedorUpdUsu").click(function(){ 
            $.ajax({
                   type: "POST",
                   url: "/Intranet/emp/EajaxProvAprobadoUpdUsu.htm",
                   data: $("#formUpdUsu").serialize(), // Adjuntar los campos del formulario enviado.
                   success: function(data)
                   {
                        if(data!="0"){
                            window.location.href = data;
			...
			jQuery(function($) {
			 ...
			 $('#contrasenna2').on('change',function(e) {
                         ...
                         $('#contrasennap').on('change',function(e) {
                             
                             if($('#contrasennap').val()==$('#contrasenna2p').val()){
                                
                                 $('#labelCoincide2').show();
                                 $('#labelNOCoincide2').hide();
                             }else if($('#contrasennap').val()!=$('#contrasenna2p').val()){
                                 
                                 $('#labelCoincide2').hide();
                                $('#labelNOCoincide2').show();
                             }
					
					
			 });
			 $('#contrasenna2p').on('change',function(e) {
                              if($('#contrasennap').val()==$('#contrasenna2p').val()){
			 ...
			 $('#fechaContrato1').datetimepicker().next().on(ace.click_event, function(){
					$(this).prev().focus();
				});
                                $('#fechaContrato2').datetimepicker().next().on(ace.click_event, function(){
					$(this).prev().focus();
				});
                                $('#fechaContrato3').datetimepicker().next().on(ace.click_event, function(){
					$(this).prev().focus();
				});
			})
		</script>
...
$(document).ready(function(){
	...
	$(".calidadtrabajo").rating();
        $(".limpiezaorden").rating();
        $(".plazos").rating();
        $(".cumplimiento").rating();
	$(".calificacion").rating();	
});
