...
<script src="<c:url value="/resources/assets/js/bootstrap.js" />"></script>
    ...
    $("#table2 tbody tr").click(function (index) 
        {
            ...
            $(this).children("td").each(function (index2) 
            {
                switch (index2) 
                {
                    case 0: idC = $(this).text();
                            break;
                    
                }
            });
           
           var json = {propuesta_id:${prop.id}
                        ,idCap:idC}; 
                    
            $.ajax({
                type: "POST",
                contentType:'application/json',
                url: "/Intranet/sup/PresupSupCap.json",
                data: JSON.stringify(json),     
                success:function(data){
                    $.each(data, function(i, item) {
                        if (item.tipoCantidad=='m2') {
                           $('#table1').append('<tr><td>'+item.subcapitulo.id+'</td><td>'+item.subcapitulo.cod+'</td><td>'+item.subcapitulo.nombre+'</td><td>'+item.tipoCantidad+'</td><td></td></tr>');
                        }else{   
                           $('#table1').append('<tr><td>'+item.subcapitulo.id+'</td><td>'+item.subcapitulo.cod+'</td><td>'+item.subcapitulo.nombre+'</td><td>'+item.tipoCantidad+'</td><td></td>-</tr>'); 
                        }
        ...
        $("#table tbody tr").click(function (index) 
        {
            var idC=null;
            $(this).children("td").each(function (index2) 
            {
